...
function visibleText($2) {
  const clone2 = $2.load($2.html());
  clone2("script, style, noscript, svg, canvas").remove();
  return clone2("body").text().replace(/\s+/g, " ").trim();
}

// ../../packages/analyzer/src/rules/agent-ux.ts
var agentUxRule = {
  id: "agent-ux",
  category: "agent-ux",
  check(ctx) {
    ...
    return [
      finding({
        ...
        category: "agent-ux",
        title: "Agent UX",
        description: `Heuristic score based on navigation, canonical URLs, structured data, metadata, landmarks, headings, and link names. Passed: ${parts.filter((p) => p.ok).length}/${parts.length}.${missing.length ? ` Gaps: ${missing.join(", ")}.` : ""} See docs/scoring.md.`,
        severity: score >= 15 ? "pass" : score >= 10 ? "info" : "warning",
        score,
        maxScore: 20,
        recommendation: missing.length > 0 ? `Improve agent UX by addressing: ${missing.join(", ")}.` : void 0
      })
    ];
  }
};

// ../../packages/analyzer/src/rules/content-access.ts
var titleRule = {
  id: "title",
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        score: inRange ? 10 : 7,
        maxScore: 10,
        recommendation: inRange ? void 0 : "Keep titles roughly 15\u201370 characters so they stay informative and scannable.",
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        score: 3,
        maxScore: 12,
        recommendation: "Include meaningful content in the first HTML response so non-browser clients can read it without executing JavaScript.",
        evidence: { value: String(length) }
      })
    ];
  }
};
var csrDetectionRule = {
  id: "csr-detection",
  category: "content-access",
  check(ctx) {
    const text3 = visibleText(ctx.$);
    const html3 = ctx.page.html;
    const emptyRoots = ["#root", "#__next", "#app", "#__nuxt"].filter((sel2) => {
      const el = ctx.$(sel2).first();
      return el.length > 0 && el.text().trim().length < 40;
    });
    const scriptCount = ctx.$("script").length;
    const looksLikeShell = text3.length < 80 && (emptyRoots.length > 0 || scriptCount >= 5 || /id=["']root["']/.test(html3));
    if (!looksLikeShell) {
      return [
        finding({
          id: "csr-detection",
          ...
          description: "The initial HTML appears to contain page content rather than an empty application shell.",
          severity: "pass",
          score: 10,
          maxScore: 10
        })
    ...
    return [
      finding({
        ...
        category: "content-access",
        title: "Initial HTML content",
        description: "Potential client-rendered content detected. The first HTML response looks like an application shell with little readable text. This is a heuristic, not a framework judgment \u2014 React and Next.js sites can still be highly readable when they ship content in HTML.",
        severity: "warning",
        score: 4,
        maxScore: 10,
        recommendation: "Server-render or prerender primary content so fetch-based crawlers and agents see text without running JavaScript.",
        evidence: {
...
// ../../packages/analyzer/src/rules/crawlability.ts
var httpStatusRule = {
  id: "http-status",
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        category: "crawlability",
        title: "HTTPS",
        description: https ? "The page is served over HTTPS." : "The page is served over HTTP. Crawlers and browsers expect TLS.",
        severity: https ? "pass" : "critical",
        score: https ? 8 : 0,
        maxScore: 8,
        recommendation: https ? void 0 : "Serve the site over HTTPS and redirect HTTP traffic.",
    ...
    if (!robots.fetched) {
      return [
        finding({
          ...
          category: "crawlability",
          title: "robots.txt",
          description: robots.statusCode ? `/robots.txt returned HTTP ${robots.statusCode}.` : "/robots.txt was not found.",
          severity: "warning",
          score: 4,
          maxScore: 10,
          recommendation: "Publish a robots.txt that declares a sitemap and avoids blocking public content."
        })
      ];
    }
...
var sitemapRule = {
  ...
  check(ctx) {
    ...
    if (!sitemap.fetched) {
      return [
        finding({
          ...
          category: "crawlability",
          title: "sitemap.xml",
          description: sitemap.declaredInRobots ? "A sitemap is declared in robots.txt but could not be fetched." : "No sitemap.xml was found at /sitemap.xml and none was declared in robots.txt.",
          severity: "warning",
          score: 3,
          maxScore: 10,
          recommendation: "Publish a sitemap.xml and declare it in robots.txt so crawlers can discover URLs."
        })
      ];
    }
    if (!sitemap.validXml) {
    ...
    const sameOriginRatio = sitemap.urlCount === 0 ? 0 : sitemap.sameOriginCount / sitemap.urlCount;
    const score = 6 + (sitemap.urlCount > 0 ? 2 : 0) + (sameOriginRatio >= 0.8 ? 1 : 0) + (sitemap.declaredInRobots ? 1 : 0);
    return [
      finding({
        id: "sitemap",
        category: "crawlability",
...
var indexabilityRule = {
  id: "indexability",
  category: "crawlability",
  check(ctx) {
    const robotsMeta = (ctx.$('meta[name="robots"]').attr("content") ?? ctx.$('meta[name="googlebot"]').attr("content") ?? "").toLowerCase();
    ...
    if (noindex || none) {
      return [
        finding({
          ...
          score: 2,
          maxScore: 8,
          recommendation: "Remove noindex if this page should be discoverable by search engines and AI crawlers.",
...
// ../../packages/analyzer/src/rules/interactive.ts
var GENERIC = /* @__PURE__ */ new Set(["click here", "submit", "ok", "button", "learn more", "read more", "more"]);
function accessibleName(ctx, el) {
  const node = ctx.$(el);
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        category: "agent-ux",
        title: "Interactive controls",
        description: issues === 0 ? `${total} button(s) have an accessible name.` : `${issues} of ${total} buttons lack a useful accessible name${examples.length ? ` (e.g. ${examples.join(", ")})` : ""}. This check uses visible text, value, title, and aria-label.`,
        severity: unnamed > 0 ? "warning" : generic > 0 ? "info" : "pass",
        score,
        maxScore: 6,
        recommendation: issues > 0 ? "Give each button a specific action name, such as \u201CStart free trial\u201D, instead of \u201CClick here\u201D or an icon with no accessible name." : void 0,
...
// ../../packages/analyzer/src/rules/links.ts
var GENERIC_LABELS = /* @__PURE__ */ new Set([
  "click here",
  "read more",
  "learn more",
  "more",
...
var linksRule = {
  ...
  check(ctx) {
    ...
    anchors.each((_, el) => {
      const href = ctx.$(el).attr("href")?.trim() ?? "";
      if (!href || href === "#") empty2 += 1;
      const name = accessibleName2(ctx.$(el));
      if (!name) {
        empty2 += 1;
        return;
      }
    ...
    let score = 10;
    if (empty2 > 0) score -= Math.min(5, empty2);
    if (genericRatio > 0.25) score -= 4;
    else if (genericRatio > 0.1) score -= 2;
    score = Math.max(0, score);
    const bits = [`${total} links.`];
    if (empty2) bits.push(`${empty2} empty or unnamed.`);
    ...
    return [
      finding({
        ...
        severity: empty2 > 3 || genericRatio > 0.25 ? "warning" : generic ? "info" : "pass",
        score,
        maxScore: 10,
        recommendation: generic || empty2 ? 'Prefer destination-specific labels such as "View pricing plans" instead of "Click here".' : void 0,
...
// ../../packages/analyzer/src/rules/llm-discoverability.ts
var llmsTxtRule = {
  id: "llms-txt",
  ...
  check(ctx) {
    ...
    if (!file.fetched) {
      return [
        finding({
          ...
          category: "llm-discoverability",
          title: "llms.txt",
          description: "No llms.txt detected. This emerging convention can provide an additional machine-readable description of your website. It is not a mandatory standard, and its absence does not mean the site is not AI-ready.",
          severity: "info",
          score: 4,
          maxScore: 10,
          recommendation: "Consider adding /llms.txt with a title, short description, and links to important docs."
        })
      ];
    }
    let score = 6;
    ...
    return [
      finding({
        ...
        category: "llm-discoverability",
        title: "llms.txt",
        description: `/llms.txt was found.${file.hasTitle ? " Title present." : ""} ${file.hasDescription ? " Description present." : ""} ${file.linkCount} markdown link(s). llms.txt is an emerging convention, not a required standard.`,
        severity: "pass",
        score: Math.min(10, score),
        maxScore: 10,
    ...
    if (!file.fetched) {
      return [
        finding({
          ...
          category: "llm-discoverability",
          title: "llms-full.txt",
          description: "No llms-full.txt detected. This optional companion file can offer a longer machine-readable summary. It is an emerging convention, not a requirement.",
          severity: "info",
          score: 3,
          maxScore: 5
        })
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        score,
        maxScore: 10,
        recommendation: restricted.length > 0 ? "Review robots.txt groups for AI crawlers if you intend those agents to access public content." : void 0
      })
    ];
  }
};

// ../../packages/analyzer/src/rules/semantic-html.ts
var LANDMARKS = ["header", "nav", "main", "article", "section", "aside", "footer"];
var headingsRule = {
  id: "headings",
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        description: `H1: ${counts.h1}  H2: ${counts.h2}  H3: ${counts.h3}${skipped ? " \u2014 heading levels skip in the outline." : " \u2014 logical heading hierarchy."}${empty2 ? ` ${empty2} empty heading(s).` : ""}`,
        severity,
        score,
        maxScore,
        recommendation,
        evidence: {
          value: JSON.stringify({ ...counts, empty: empty2, skipped })
        }
      })
    ];
  }
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        description: present.length ? `Found ${present.length}/${LANDMARKS.length} landmark elements: ${present.join(", ")}.` : "No HTML5 landmark elements (header, nav, main, article, section, aside, footer) were found.",
        severity: hasMain && present.length >= 3 ? "pass" : present.length > 0 ? "info" : "warning",
        score: hasMain ? score : Math.max(0, score - 2),
        maxScore: 10,
        recommendation: hasMain && present.length >= 3 ? void 0 : "Use landmark elements such as <main>, <nav>, and <header> so agents can identify page regions.",
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        category: "semantic-html",
        title: "Image alt text",
        description: missing === 0 ? `All ${total} image(s) have an alt attribute.` : `${missing} of ${total} images are missing an alt attribute. This check only inspects the alt attribute; it does not certify accessibility.`,
        severity: missing === 0 ? "pass" : missing / total > 0.4 ? "warning" : "info",
        score,
        maxScore: 8,
        recommendation: missing > 0 ? 'Add alt text that describes the image, or alt="" for decorative images.' : void 0,
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        category: "semantic-html",
        title: "Forms",
        description: unlabeled === 0 ? `${forms.length} form(s) and ${inputs} field(s) have associated labels.` : `${unlabeled} of ${inputs} fields appear unlabeled. This is a heuristic based on label[for], wrapping <label>, and aria-label.`,
        severity: unlabeled === 0 ? "pass" : "warning",
        score,
        maxScore: 4,
        recommendation: unlabeled > 0 ? "Associate each form control with a <label> or an accessible name so agents can submit forms predictably." : void 0
      })
    ];
  }
};

// ../../packages/analyzer/src/rules/structured-data.ts
var RECOGNIZED_TYPES = /* @__PURE__ */ new Set([
  "Organization",
  "WebSite",
  "WebPage",
  "Article",
...
function typesOf(node) {
  ...
  if (Array.isArray(value)) {
    return value.filter((v) => typeof v === "string").map((v) => v.split("/").pop() ?? v);
  }
  return [];
}
  ...
  check(ctx) {
    ...
    if (scripts.length === 0) {
      return [
        finding({
          ...
          score: 4,
          maxScore: 20,
          recommendation: "Add JSON-LD for Organization, WebSite, or WebPage so agents can identify the entity behind the URL."
        })
      ];
    }
    const nodes = [];
    ...
    for (const node of nodes) {
      for (const type of typesOf(node)) {
        ...
        for (const field of required) {
          if (node[field] === void 0) missingFields.push(`${type}.${field}`);
        }
      }
    }
    ...
    return [
      finding({
        ...
        category: "structured-data",
        title: "JSON-LD",
        description: `Parsed ${nodes.length} node(s). Types: ${typeList}.${malformed ? ` ${malformed} malformed block(s).` : ""}${missingFields.length ? ` Missing basic fields: ${missingFields.slice(0, 5).join(", ")}.` : ""} This check is not complete Schema.org validation.`,
        severity: malformed || missingFields.length ? "info" : "pass",
        score: malformed ? Math.min(score, 12) : score,
        maxScore: 20,
        recommendation: recognized.length === 0 ? "Include a recognized type such as Organization, WebSite, or Article." : void 0,
...
// ../../packages/analyzer/src/rules/technical-seo.ts
var viewportRule = {
  id: "viewport",
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        score: viewport ? 4 : 2,
        maxScore: 4,
        recommendation: viewport ? void 0 : 'Add <meta name="viewport" content="width=device-width, initial-scale=1">.',
        evidence: viewport ? { selector: 'meta[name="viewport"]', value: viewport } : void 0
      })
    ];
  }
};
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        category: "technical-seo",
        title: "Open Graph",
        description: present === 0 ? "No Open Graph tags (og:title, og:description, og:url) were found." : `Open Graph: ${present}/3 core tags present.`,
        severity: present >= 2 ? "pass" : present === 1 ? "info" : "warning",
        score: present === 0 ? 2 : present * 2,
        maxScore: 6,
        recommendation: present < 3 ? "Add og:title, og:description, and og:url for richer unfurls." : void 0
      })
    ];
  }
};
  ...
  check(ctx) {
    ...
    return [
      finding({
        ...
        category: "technical-seo",
        title: "Robots meta",
        description: robots ? `Robots meta is set to "${robots}".` : "No robots meta tag; crawlers typically default to index, follow.",
        severity: "pass",
        score: 3,
        maxScore: 3,
        evidence: robots ? { selector: 'meta[name="robots"]', value: robots } : void 0
      })
    ];
  }
};
var responseHygieneRule = {
  id: "response-hygiene",
  category: "technical-seo",
  check(ctx) {
    ...
    const html3 = contentType.includes("html") || ctx.page.html.includes("<html");
    let score = 4;
    if (ms > 5e3) score -= 2;
    else if (ms > 2e3) score -= 1;
    if (bytes > 15e5) score -= 1;
    if (!html3) score -= 1;
    return [
      finding({
        id: "response-hygiene",
        ...
        description: `Fetched in ${ms}ms (${bytes} bytes)${html3 ? ", HTML content type." : "."}`,
...
// ../../packages/analyzer/src/evidence.ts
var GENERIC_LABELS2 = /* @__PURE__ */ new Set([
  "click here",
  "read more",
  "learn more",
  "more",
...
function uniqueLineOf(source, needle) {
  if (!needle || needle.length < 3) return void 0;
  const index2 = source.indexOf(needle);
  if (index2 < 0) return void 0;
  if (source.indexOf(needle, index2 + 1) !== -1) return void 0;
  return source.slice(0, index2).split(/\r?\n/).length;
}
function clip(text3, max = 480) {
  const trimmed = text3.trim();
  if (trimmed.length <= max) return trimmed;
  return `${trimmed.slice(0, max)}\u2026`;
}
function elementSnippet(ctx, selector) {
  const node = ctx.$(selector).first();
  if (node.length === 0) return void 0;
  const html3 = node.toString();
  return html3 ? clip(html3) : void 0;
}
function lineInHtml(ctx, needle) {
  return uniqueLineOf(ctx.page.html, needle);
}
function isGenericLabel(text3) {
  return GENERIC_LABELS2.has(text3.replace(/\s+/g, " ").trim().toLowerCase());
}
var WHY = {
  "http-status": "Non-200 responses stop most fetch-based crawlers and agents from reading the page at all.",
  https: "TLS is a baseline expectation for crawlers, browsers, and automated clients that refuse mixed or plaintext origins.",
  redirects: "Long redirect chains waste crawl budget and can hide the canonical destination from simpler agents.",
  "robots-txt": "robots.txt is the first machine-readable policy file crawlers request. Syntax errors or a missing file reduce how clearly you declare crawl intent.",
  sitemap: "A sitemap helps crawlers discover URLs that are not linked from the homepage.",
  canonical: "A canonical URL tells machines which address is the preferred version of this page when duplicates or tracking variants exist.",
  indexability: "noindex signals tell compliant crawlers not to include the page in their index.",
  title: "The document title is one of the strongest short labels a machine can use to name the page.",
  description: "A meta description gives crawlers and unfurlers a concise summary when they cannot infer one from the body.",
  "text-content": "Fetch-based agents read the first HTML response. Very little visible text means they see less of the page without executing JavaScript.",
  "csr-detection": "An empty application shell in the first HTML response is a heuristic that content may depend on client rendering. It does not mean agents cannot access the site.",
  headings: "A single H1 and a logical H2/H3 outline give agents a page hierarchy they can follow without layout cues.",
  landmarks: "Landmark elements such as main and nav identify page regions for machines and assistive tools.",
  "images-alt": "Alt text is the machine-readable name of an image. This check only inspects the alt attribute.",
  forms: "Labeled fields let agents understand what a form collects before submitting it.",
  "json-ld": "JSON-LD can identify the entity, page type, and relationships behind a URL. This is not complete Schema.org validation.",
  "llms-txt": "llms.txt is an emerging convention for a machine-readable site description. It is not a mandatory web standard, and its absence does not mean a site is not AI-ready.",
  "llms-full-txt": "llms-full.txt is an optional companion to llms.txt. It is an emerging convention, not a requirement.",
  "ai-crawlers": "Dedicated robots.txt groups can restrict named AI crawlers. Unspecified does not guarantee access; crawler behavior can change independently.",
  links: "Descriptive link text gives agents destination context. Generic phrases are not automatically a failure.",
  "agent-ux": "Agent UX is a documented heuristic over navigation, canonical URLs, structured data, metadata, landmarks, headings, and link names.",
  interactive: "Buttons and other controls need an accessible name so agents can tell what action they perform. Icon-only controls without aria-label are ambiguous.",
  viewport: "A viewport tag is a basic document hint used by browsers and some renderers.",
  "open-graph": "Open Graph tags provide a stable title, description, and URL for unfurls and some machine clients.",
  "twitter-meta": "Twitter/X card tags are optional social metadata. Their absence is informational, not a crawl blocker.",
  "robots-meta": "A robots meta tag, when present, is an additional indexability signal on the page itself.",
  "response-hygiene": "Very slow or extremely large first responses make fetch-based analysis and crawling less reliable."
};
function defaultFix(ctx, finding2) {
...
function enhanceEvidence(finding2, ctx) {
  const current = finding2.evidence ?? {};
  const selector = current.selector;
  const snippet = current.snippet ?? (selector ? elementSnippet(ctx, selector) : void 0) ?? current.value;
  ...
  } else if (current.url?.endsWith("robots.txt") || finding2.id === "robots-txt" || finding2.id === "ai-crawlers") {
    source = source ?? "robots.txt";
    location = location ?? "robots.txt";
    precision = precision ?? (line ? "line" : "document");
  } else if (finding2.id === "sitemap") {
    source = source ?? "sitemap.xml";
    location = location ?? "sitemap.xml";
    precision = precision ?? "document";
  } else if (finding2.id.startsWith("llms")) {
    source = source ?? finding2.id === "llms-full-txt" ? "/llms-full.txt" : "/llms.txt";
...
function enrichFinding(finding2, ctx) {
  const evidence = enhanceEvidence(finding2, ctx);
  return {
    ...finding2,
    evidence,
    whyItMatters: finding2.whyItMatters ?? WHY[finding2.id],
    recommendedFix: finding2.recommendedFix ?? defaultFix(ctx, finding2),
    detectedSnippet: finding2.detectedSnippet ?? (finding2.id === "links" && evidence.value && !evidence.value.startsWith("http") ? `<a href="/\u2026">${evidence.value.split(",")[0]?.trim()}</a>` : evidence.snippet),
    affectedPages: finding2.affectedPages ?? [ctx.page.finalUrl]
  };
}

// ../../packages/analyzer/src/pages.ts
function pageHasIssue(id, page) {
  switch (id) {
    case "canonical":
    ...
    case "images-alt":
      return page.$("img").toArray().some((el) => page.$(el).attr("alt") === void 0);
    case "links": {
      let generic = 0;
      const anchors = page.$("a[href]");
      anchors.each((_, el) => {
        const text3 = (page.$(el).attr("aria-label") ?? page.$(el).text()).replace(/\s+/g, " ").trim();
        if (isGenericLabel(text3)) generic += 1;
      });
...
function attachAffectedPages(findings, crawl) {
  const homepage = crawl.homepage.finalUrl;
  if (crawl.extraPages.length === 0) {
    return findings.map((finding2) => ({
      ...finding2,
      affectedPages: finding2.affectedPages ?? [homepage]
    }));
  }
  return findings.map((finding2) => {
    const extras = crawl.extraPages.filter((page) => pageHasIssue(finding2.id, page)).map((page) => page.finalUrl);
    const unique = [.../* @__PURE__ */ new Set([homepage, ...extras])];
    return { ...finding2, affectedPages: unique };
  });
}

// ../../packages/analyzer/src/run.ts
async function runRules(crawl) {
  const ctx = {
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/static.js
var static_exports = {};
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/options.js
var defaultOpts2 = {
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/api/attributes.js
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/utils.js
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/api/traversing.js
...
function compileRules(rules2, options, context) {
  var _a5;
  return rules2.reduce((previous, rule) => previous === import_boolbase5.default.falseFunc ? import_boolbase5.default.falseFunc : compileGeneralSelector(previous, rule, options, context, compileToken), (_a5 = options.rootFunc) !== null && _a5 !== void 0 ? _a5 : import_boolbase5.default.trueFunc);
}
...
function some(elements, selector, options = {}) {
  ...
  return plain.length > 0 && elements.some(_compileToken(plain, options)) || filtered.some((sel2) => filterBySelector(sel2, elements, options).length > 0);
}
...
function select(selector, root2, options = {}, limit = Infinity) {
  ...
  const [plain, filtered] = groupSelectors(parse(selector));
  const results = filtered.map((sel2) => findFilterElements(root2, sel2, options, true, limit));
  if (plain.length) {
...
function findElements(root2, sel2, options, limit) {
  const query = _compileToken(sel2, options, root2);
...
function filterElements(elements, sel2, options) {
  const els = (Array.isArray(elements) ? elements : [elements]).filter(isTag2);
  if (els.length === 0)
    return els;
  const query = _compileToken(sel2, options);
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/api/manipulation.js
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/parse.js
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/api/css.js
var css_exports = {};
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/api/forms.js
var forms_exports = {};
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/api/extract.js
var extract_exports = {};
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/cheerio.js
var Cheerio = class {
  /**
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/load.js
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/parsers/parse5-adapter.js
...
// ../../node_modules/.pnpm/cheerio@1.2.0/node_modules/cheerio/dist/browser/load-parse.js
...
function crawlerStatus(groups, name) {
  const ua = name.toLowerCase();
  const specific = groups.filter(
    (g) => g.userAgents.some((agent) => agent.toLowerCase() === ua)
  );
...
function analyzeRobotsTxt(text3, statusCode) {
  if (text3 === null) {
    return {
      fetched: false,
      statusCode,
  ...
  const parsed = parseRobotsTxt(text3);
  return {
    fetched: true,
    statusCode,
    ...
    crawlers: KNOWN_AI_CRAWLERS.map((name) => {
      ...
      const located = locateUserAgent(text3, name);
      return located.line || located.snippet ? { ...access, ...located } : access;
    })
  };
}
function locateUserAgent(text3, name) {
  const lines = text3.split(/\r?\n/);
...
function parseSitemapXml(xml2, _originUrl) {
  ...
  locNodes.each((_, el) => {
    ...
    const lastmod = $2(el).parent().find("lastmod").first().text().trim() || void 0;
...
function analyzeSitemap(xml2, originUrl2, options) {
  if (xml2 === null) {
    return {
      fetched: false,
  ...
  const parsed = parseSitemapXml(xml2, originUrl2);
  const sameOriginCount = parsed.urls.filter((u) => isSameOrigin(u.loc, originUrl2)).length;
...
function emptyLlmsTxt(path, statusCode) {
  return {
    path,
    fetched: false,
    statusCode,
...
// ../../packages/analyzer/src/inspect.ts
...
function elementPath(ctx, el) {
  ...
  if (id) {
    const selector2 = `#${escapeIdent(id)}`;
    return { selector: selector2, unique: ctx.$(selector2).length === 1 };
  }
  const parts = [];
  let node = el;
  while (isElement(node)) {
    ...
    const siblings2 = sel(ctx, parentNode).children(tag).toArray();
    const index2 = siblings2.indexOf(node) + 1;
    parts.unshift(siblings2.length > 1 ? `${tag}:nth-of-type(${index2})` : tag);
    node = parentNode;
  }
...
function targetFrom(ctx, el, ruleId) {
  const located = elementPath(ctx, el);
  const node = sel(ctx, el);
  const text3 = node.text().replace(/\s+/g, " ").trim().slice(0, 80) || void 0;
...
var GENERIC_BUTTONS = /* @__PURE__ */ new Set(["click here", "submit", "ok", "button", "learn more", "read more"]);
function accessibleName3(ctx, el) {
  const node = sel(ctx, el);
  const aria = node.attr("aria-label")?.trim();
  if (aria) return aria;
  if (node.attr("aria-labelledby")?.trim()) return node.attr("aria-labelledby").trim();
...
function collectInspectTargets(ctx, limit = 8) {
  const targets = [];
  const push = (item) => {
  ...
  ctx.$("a[href]").each((_, el) => {
    ...
    const href = ctx.$(el).attr("href")?.trim() ?? "";
    const name = accessibleName3(ctx, el);
  ...
  ctx.$("img").each((_, el) => {
    if (!isElement(el)) return;
    if (ctx.$(el).attr("alt") === void 0) push(targetFrom(ctx, el, "images-alt"));
  });
  ctx.$("input, select, textarea").each((_, el) => {
    if (!isElement(el)) return;
    const type = (ctx.$(el).attr("type") ?? "text").toLowerCase();
...
function attachInspectTargets(findings, targets) {
  return findings.map((finding2) => {
    const matched = targets.filter((item) => item.ruleId === finding2.id);
    if (matched.length === 0) return finding2;
    ...
    return {
      ...
      evidence: {
        ...finding2.evidence,
        selector: finding2.evidence?.selector ?? unique?.selector,
        snippet: finding2.evidence?.snippet ?? unique?.snippet,
        tagName: unique?.tagName,
        text: unique?.text,
        inspectable: matched.some((item) => item.unique),
        precision: unique?.unique ? "selector" : finding2.evidence?.precision
      }
    };
  });
}
function mergeInspectTargets(generated, live) {
...
// ../../packages/analyzer/src/seo-inspection.ts
var SEO_GENERIC = /* @__PURE__ */ new Set([
  "click here",
  "here",
  "learn more",
  "read more",
...
function ctxOf(page, crawl) {
  return { url: page.url, page, crawl, $: page.$ };
}
function originUrl(pageUrl, path) {
  try {
...
function fileStatus(fetched, statusCode, body) {
  if (fetched && body) return "found";
  if (statusCode === 404) return "not-found";
  if (statusCode === 0) return "error";
  if (statusCode !== void 0 && statusCode >= 400) return "error";
  if (!fetched && statusCode === void 0 && !body) return "unknown";
  return "not-found";
}
function locate(ctx, el) {
  const located = elementPath(ctx, el);
  return located.selector ? located : {};
}
function accessibleName4($2, el) {
  const node = $2(el);
  const aria = node.attr("aria-label")?.trim();
  if (aria) return aria;
  const imgAlt = node.find("img[alt]").first().attr("alt")?.trim();
...
function isSeoGeneric(text3) {
  return SEO_GENERIC.has(text3.replace(/\s+/g, " ").trim().toLowerCase()) || isGenericLabel(text3);
}
...
function titleSnapshot(page) {
  const text3 = page.$("title").first().text().replace(/\s+/g, " ").trim();
  if (!text3) {
    return { status: "fail", notes: ["Missing title"], length: 0 };
  }
  const notes = ["Title exists"];
  let status = "pass";
  if (text3.length > 70) {
    status = "warning";
    notes.push("Title is very long");
  }
  return { value: text3, length: text3.length, status, notes };
}
function descriptionSnapshot(page) {
  const text3 = page.$('meta[name="description"]').first().attr("content")?.replace(/\s+/g, " ").trim() ?? "";
  ...
  const notes = ["Description exists"];
  let status = "pass";
  if (text3.length > 160) {
    status = "warning";
...
function canonicalSnapshot(page) {
...
function languageSnapshot(page) {
...
function buildSeoSnapshot(page) {
  return {
...
function buildHeadingInspection(page, crawl) {
  ...
  page.$("h1, h2, h3, h4, h5, h6").each((_, el) => {
    ...
    const text3 = page.$(el).text().replace(/\s+/g, " ").trim();
    ...
    if (headingCounts[tag] !== void 0) headingCounts[tag] += 1;
  });
...
function buildLinkIndex(page, crawl) {
  const ctx = ctxOf(page, crawl);
  const issues = [];
  const items = [];
  let generic = 0;
  ...
  anchors.each((_, el) => {
    ...
    const text3 = accessibleName4(page.$, el);
    ...
    if (/^javascript:/i.test(href)) {
      ...
      kind = "empty";
      empty2 += 1;
    } else if (!text3 && hasImage) {
      kind = "image-only";
      imageOnly += 1;
    } else if (!text3) {
      kind = "empty";
      empty2 += 1;
    } else if (isSeoGeneric(text3)) {
      kind = "generic";
      generic += 1;
    }
    try {
    ...
    const record = {
      ...
      rel,
      internal: origin,
      nofollow: isNofollow || void 0,
      sponsored: isSponsored || void 0,
      ugc: isUgc || void 0,
      kind,
      selector: located.selector,
      unique: located.unique
  ...
  return {
    links: {
      total,
      descriptive: Math.max(0, total - generic - empty2),
      generic,
      empty: empty2,
      issues
    },
...
function buildImageIndex(page, crawl) {
  const ctx = ctxOf(page, crawl);
  const issues = [];
  const items = [];
  let missingAlt = 0;
  ...
  images.each((_, el) => {
    const node = page.$(el);
    const src = node.attr("src") ?? void 0;
...
function buildJsonLd(page, crawl) {
  const ctx = ctxOf(page, crawl);
  const blocks = [];
  page.$('script[type="application/ld+json"]').each((index2, el) => {
    ...
    try {
      ...
      const walk = (node) => {
        ...
        if (types.includes("Organization") && record.url === void 0) missing.push("Organization.url");
        if (types.includes("WebSite") && record.name === void 0) missing.push("WebSite.name");
      };
      walk(parsed);
      blocks.push({
        index: index2,
...
function buildSchemaSnapshot(page, jsonLd) {
...
function buildSitemapSnapshot(crawl, pageUrl, canonicalHref) {
...
function buildRobotsDetail(crawl, pageUrl) {
  return {
...
// ../../packages/analyzer/src/inspection.ts
var LANDMARKS2 = ["header", "nav", "main", "article", "section", "aside", "footer"];
function pageRecord(page) {
  ...
  return {
    ...
    title: page.$("title").first().text().trim() || void 0
  };
}
function buildInspection(crawl) {
  const page = crawl.homepage;
  const landmarks = {};
  for (const tag of LANDMARKS2) {
  ...
  return {
    ...
    robotsRaw: crawl.robotsBody ?? void 0,
    sitemapRaw: crawl.sitemapBody ?? void 0,
    llmsRaw: crawl.llmsTxtBody ?? void 0,
    llmsFullRaw: crawl.llmsFullTxtBody ?? void 0,
...
function overallFrom(findings) {
  const weighted = CATEGORIES.reduce((sum, id) => {
    return sum + categoryScore(findings, id) * CATEGORY_META[id].weight;
  }, 0);
  return clamp(Math.round(weighted), 0, 100);
}
function projectScore(findings, passIds) {
  const projected = findings.map(
    (finding2) => passIds.includes(finding2.id) ? { ...finding2, score: finding2.maxScore, severity: "pass" } : finding2
  );
  return overallFrom(projected);
}
function attachScoreImpact(findings) {
  const base = overallFrom(findings);
  return findings.map((finding2) => {
    if (finding2.severity === "pass" || finding2.score === finding2.maxScore) {
      return { ...finding2, scoreImpact: 0 };
    }
    return { ...finding2, scoreImpact: projectScore(findings, [finding2.id]) - base };
  });
}
function buildBreakdown(findings, categories) {
  return categories.map((category) => {
    const checks = findings.filter((f) => f.category === category.id).map((f) => ({
      id: f.id,
      title: f.title,
      score: f.score,
      maxScore: f.maxScore,
...
function gradeNarrative(score, grade) {
...
function buildInsights(findings, categories, score, grade) {
  ...
  const topActions = findings.filter((f) => f.severity === "critical" || f.severity === "warning").slice().sort((a, b) => {
...
function scoreFindings(findings) {
  ...
  const recommendations = withImpact.filter((f) => f.recommendation && f.severity !== "pass").map((f) => ({
    id: f.id,
    title: f.title,
...
// ../../packages/analyzer/src/snapshot.ts
function utf8Bytes(text3) {
  return new TextEncoder().encode(text3).byteLength;
}
function crawlFromCollected(input) {
  const url = input.url;
  const html3 = input.html;
  const $2 = load(html3);
  const page = {
    url,
    finalUrl: url,
  ...
  return {
    ...
    crawler: {
      robotsTxt: robots,
      sitemap,
      llmsTxt: llmsBody ? parseLlmsTxt(llmsBody, "/llms.txt", input.llmsTxt?.status) : emptyLlmsTxt("/llms.txt", input.llmsTxt?.status),
      llmsFullTxt: llmsFullBody ? parseLlmsTxt(llmsFullBody, "/llms-full.txt", input.llmsFullTxt?.status) : emptyLlmsTxt("/llms-full.txt", input.llmsFullTxt?.status)
    },
    robotsBody,
    sitemapBody,
...
function countDomStats(html3, live) {
  if (live) return live;
  const $2 = load(html3);
  return {
    links: $2("a[href]").length,
    buttons: $2("button, [role='button'], input[type='button'], input[type='submit']").length,
    images: $2("img").length,
    headings: $2("h1, h2, h3, h4, h5, h6").length,
    forms: $2("form").length,
    jsonLd: $2('script[type="application/ld+json"]').length,
    inputs: $2("input:not([type='hidden']), select, textarea").length
  };
}
async function analyzeCollectedPage(input) {
...
// src/lib/restricted.ts
var BLOCKED = /^(chrome|edge|about|chrome-extension|moz-extension|devtools|view-source|file):/i;
var STORES = [
...
function restrictedError(url) {
  ...
  return {
    title: "AgentLens can't inspect this page.",
    reason: url ? "Chrome does not allow extensions to inspect browser-internal or store pages." : "This tab has no inspectable address.",
    hint: "Open a public http(s) website, then click AgentLens again."
  };
}
...
async function collect(tabId) {
  ...
  const response = await chrome.tabs.sendMessage(tabId, { type: COLLECT_TYPE });
...
async function analyze(port, force) {
    ...
    void (async () => {
      try {
        ...
        if (message.type === "GET_STATE") {
          const tab = await activeTab();
          const entry = tab.id ? cache.get(tab.id) : void 0;
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.type === "GET_LAST_RESULT") {
    sendResponse({ type: "LAST_RESULT", result: lastResult });
    return false;
  }
  ...
  if (message.type === "INSPECT") {
    void (async () => {
      ...
      if (!tabId) {
        sendResponse({ found: false, reason: "No active tab." });
        return;
      }
      await ensureContent(tabId);
      const payload = message.payload;
      const result = await chrome.tabs.sendMessage(tabId, { type: INSPECT_TYPE, payload });
      sendResponse(result);
    })();
    return true;
  }
